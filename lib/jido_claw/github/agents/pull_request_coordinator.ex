defmodule JidoClaw.GitHub.Agents.PullRequestCoordinator do
  ...
  defp do_attempt(event, triage, research, attempt, history) do
    Logger.info("[PRCoordinator] Attempt #{attempt}/#{@max_attempts} for #{event.repo.full_name}##{event.issue.number}")
    ...
    with {:ok, patch} <- generate_patch(event, triage, research),
      ...
      {:error, {:quality_failed, reason}} ->
        ...
        do_attempt(event, triage, research, attempt + 1, [%{attempt: attempt, error: reason} | history])
  ...
  defp generate_patch(event, _triage, _research) do
    {:ok, %{files: [], description: "Fix for ##{event.issue.number}", branch: "fix/issue-#{event.issue.number}"}}
  end
